...
Node* deleteTelNum(Node* head, char* delname) {
	...
	if (current->next == NULL) {
		printf("이름 %s를 가진 전화번호를 찾을 수 없습니다.\n",delname);
		return head;
	}
...
int main() {
	...
	while (1) {
		...
		switch (menu) {
		case 1:
			printf("추가할 연락처의 이름을 입력하시오 : ");
			scanf("%s", name);
			printf("전화번호를 입력하시오 : ( ex)ooo-oooo-oooo ) : ");
			scanf("%s", telnum);
			head = insertTelNum(head, name, telnum);
			break;
		case 2:
			printf("검색할 연락처의 이름을 입력하시오 : ");
			scanf("%s", name);
			searchTelNum(head, name);
			break;
		case 3:
			printf("삭제할 연락처의 이름을 입력하시오 : ");
			scanf("%s", name);
